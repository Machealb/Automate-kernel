function [k_corner,info] = Lcurve_corner(rho,eta,fig,rkhs_type)
...
% the original p = min(5, nP-1) not work well sometimes!
p = min(7, nP-1);           % Number of vectors in pruned L-curve.
...
if convex==0
	...
	k_corner = 1;
    fprintf('[Empty value of k_corner, set at 1]\n');
	return
end
...
if fig  % Show log-log L-curve and indicate the found corner.
	...
	xlabel('|| A x - b ||_2')
	ylabel('||x ||_{RKHS}');
    title(sprintf('L-curve for %s, corner at %d', rkhs_type,k_corner));
end
