function [basis_funs,basisInfo] = Basis_fourier(K, odd_even, supp,jump_discontinuous)
...
%     ddindicator = @(x) zeros(size(x));
    ...
    if jump_discontinuous ==1
    indicator = @(x) (x>supp(1)+.5 & x<supp(end) -1.2);
    end 
    dindicator = @(x) zeros(size(x)) ;   % TBD
    ddindicator = @(x) zeros(size(x));
...
switch odd_even
    case 'sincos'
        for i = 1:s
            ...
            ddbasis_funs{i,1} = @(x) -(i*omega)^2*cos(i*x*omega).*indicator(x) -i*omega*sin(i*x*omega).*dindicator(x) -i*omega*sin(i*x*omega).*dindicator(x)+ cos(i*x*omega).*ddindicator(x);
